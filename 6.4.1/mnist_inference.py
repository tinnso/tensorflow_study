...
def inference(input_tensor, train, regularizer):
    # 声明第一层卷积层的变量并实现前向传播过程。这个过程和6. 3 .1 节中介绍的一致。
    ...
    with tf .variable_scope('layerl-convl'):
        ...
        conv1_biases = tf.get_variable(
            "bias", [CONVl_DEEP], initializer=tf.constant_initializer(0.0))
    ...
    # 实现第二以池化版的前向传播过程。这里选用最大池化层，池化层过滤器的边长为2,
    ...
    with tf.variable_scope('layer3-conv2'):
        conv2_weights = tf.get_variable(
            "weight", [CONV2_SIZE, CONV2_SIZE, CONVl_DEEP, CONV2_DEEP],
            initializer=tf.truncated_normal_initializer(stddev=0.1))
